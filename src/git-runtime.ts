...
export function createGitRunner(ctx: Context): GitRunner {
  return async command => {
    ...
    const collect = async (stream: typeof child.stdout): Promise<Buffer> => {
      if (stream === undefined) throw new Error('subprocess did not expose a requested binary pipe')
      const parts: Buffer[] = []
      let bytes = 0
    ...
    try {
      ...
      signal.throwIfAborted()
      if (stdinError !== undefined && outcome.exitCode === 0) throw stdinError
