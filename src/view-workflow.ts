/** Research presentation mapped to the native plugin's workflow v2 input. */
import type { JsonValue } from '@deepseek-ai/dsh-util-values'
import { viewExcerpt } from './view-projection.ts'
import type { ResearchViewSnapshot } from './view-types.ts'
...
const rowId = (row: number): string => row === 0 ? 'research' : 'research_' + row

/** Keep the fixed-width card legible; full authored names remain in node details. */
function experimentLabel(prefix: string, title: string): string {
  const chars = Array.from((prefix + ' ' + title).replace(/\s+/gu, ' ').trim())
  const units = (char: string) => /[^\u0020-\u007e]/u.test(char) ? 2 : 1
  if (chars.reduce((sum, char) => sum + units(char), 0) <= 21) return chars.join('')
  let label = ''; let width = 0
  for (const char of chars) {
    if (width + units(char) > 19) break
    label += char; width += units(char)
  }
  return label.trimEnd() + '…'
}

/** Wrap logical plan/Run columns into six-column lanes with separate experiment slots. */
export function researchWorkflow(snapshot: ResearchViewSnapshot, locale: 'en' | 'zh-CN'): Record<string, JsonValue> {
