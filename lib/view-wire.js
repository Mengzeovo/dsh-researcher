...
import { z } from 'zod';
import { planMetadataSchema, planVersionRefSchema } from "./plan-schema.js";
import { researchIdSchema, researchStatusSchema, UUID_PATTERN } from "./wire.js";
...
const nodeId = z.string().refine(value => /^plan_[1-9][0-9]*_v[1-9][0-9]*$/u.test(value) || (value.startsWith('run_') && UUID_PATTERN.test(value.slice(4))), 'invalid research node id').transform(value => value);
...
const selection = z.object({ planId: positive.nullable(), versionPage: page, runPages }).strict();
...
const baseNode = { id: nodeId, planId: positive, revision: positive, title: z.string(), summary: z.string(), path: z.string(), createdAt: z.string(), column: page.max(5), slot: page.max(3) };
...
export const researchViewRequestSchema = z.object({ sessionId, planId: positive.optional(), versionPage: page.optional(), runPages: runPages.optional(), ifNoneMatch: snapshotId.optional(), refresh: z.boolean().optional() }).strict();
export const researchViewWatchRequestSchema = z.object({ sessionId }).strict();
export const researchViewNodeRequestSchema = z.object({ sessionId, snapshotId, nodeId }).strict();
export const researchViewRenderRequestSchema = z.object({ sessionId, snapshotId, theme: z.enum(['light', 'dark']), locale: z.enum(['en', 'zh-CN']) }).strict();
export const researchViewResponseSchema = z.discriminatedUnion('kind', [
    ...
    z.object({
        ...
        selection, pages: z.object({ versionPages: positive, versionsPerPage: positive.max(3), runsPerVersionPage: positive.max(4), runCounts: z.record(z.string(), page) }).strict(),
...
export const researchViewDetailSchema = z.discriminatedUnion('kind', [
...
export const researchViewRenderedSchema = z.object({ html: z.string(), svg: z.string(), revision: artifactId, nodeIds: z.array(nodeId), specSha256: hash, engineFingerprint: hash }).strict();
export const researchViewChangedSchema = z.object({ targetToken }).strict();
//# sourceMappingURL=view-wire.js.map