...
import { ResearchBriefings } from "./briefing.js";
import { assertSessionEventAccess, sessionEventAt, sessionNextSeq } from "./session-events.js";
import { nowIso, parseResearchId, researchBindingSchema, researchTargetListRequestSchema } from "./schema.js";
import { ResearchStore } from "./research-store.js";
import { resolveResearcherConfig, researcherConfigSchema } from "./view-config.js";
import ResearchViewService from "./view-service.js";
...
const bindingWireViews = new WeakMap();
function bindingWireView(state) {
...
export const researcherBindingProjectionDefinition = {
    ...
    init: (header) => ({ sessionId: String(header.id), bindings: {}, failure: null }),
...
export class ResearcherService extends TypertRemoteService {
    ...
    constructor(ctx, config) {
        ...
        if (resolved.view.enabled)
            ctx.plugin(ResearchViewService, resolved.view);
    }
    ...
    async viewWorkspace(context) { return await this.store.canonicalWorkspace(context); }
    /** Read verified graph records without agent lookup, activation, or authority mutation. */
    async viewData(context, id, config, signal) {
        return await this.store.readViewData(context, id, config, signal);
    }
    async publishMutation(session, id, operation) {
    ...
    async createPlan(agent, request, signal) {
    ...
    async activate(agent, initialTarget, signal) {
        ...
        if (initialTarget.recovery === undefined)
        ...
        try {
            ...
            throw new ResearcherError(`research target ${target.id} was loaded and injected, but its DSH Goal could not be activated; use /research-start to retry explicitly`, 'RESEARCH_GOAL_CONFLICT', { cause: error });
        }
        return { researchId: target.id, eventSeq, target, context, goalAction };
    }
    injectContext(agent, target, context, workspaceRoot) {
