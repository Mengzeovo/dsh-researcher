...
import { parsePlanDocument, parsePlanLedger, planDirectory, planLedgerPath, planRoot, planVersionPath } from "./plan-records.js";
...
import { isCheckpointRunDescription } from "./types.js";
...
export class RecordStore {
    ...
    async readStateLog(session, id, signal, maxBytes) {
    ...
    async listPlanFiles(session, id, planId, signal) {
    ...
    async readPlanLedger(session, id, planId, signal, maxBytes) {
        await this.assertPlanDirectories(session, id, planId, signal);
    ...
    async readPlanDocument(session, id, planId, revision, signal) {
        await this.assertPlanDirectories(session, id, planId, signal);
    ...
    async findPlanDocument(session, id, planId, revision, signal) {
        await this.assertPlanDirectories(session, id, planId, signal);
        const relative = planVersionPath(id, planId, revision);
        const info = await this.ctx.fs.lstat(relative, { cwd: sessionCwd(session) }, signal);
        if (info === undefined)
