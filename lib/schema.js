...
export const reproductionSchema = z.object({
    ...
    for (const item of value.snapshot?.externalInputs ?? []) {
        if (value.inputs.includes(item.path))
            ctx.addIssue({ code: 'custom', message: 'external input cannot also be a captured explicit input' });
    }
});
export const inputCheckpointSchema = z.object({
    ...
    if (value.snapshot) {
        if (value.snapshot.deleted.some(file => !value.files.includes(file)))
            ctx.addIssue({ code: 'custom', message: 'overlay deletion outside frozen files' });
        if (value.snapshot.omittedChanges.some(file => value.files.includes(file)))
            ctx.addIssue({ code: 'custom', message: 'omitted change also captured' });
        if (JSON.stringify([...value.snapshot.omittedChanges].sort()) !== JSON.stringify([...(value.reproduction.snapshot?.omitChanges ?? [])].sort()))
            ctx.addIssue({ code: 'custom', message: 'omitted changes were not explicitly acknowledged' });
        const scopes = value.reproduction.snapshot?.paths ?? [];
        if (value.files.some(file => !value.reproduction.inputs.includes(file) && !scopes.some(scope => file === scope || file.startsWith(scope + '/'))))
            ctx.addIssue({ code: 'custom', message: 'captured file outside declared scope and explicit inputs' });
        const external = new Set((value.reproduction.snapshot?.externalInputs ?? []).map(item => item.path));
        if (value.files.some(file => external.has(file)))
            ctx.addIssue({ code: 'custom', message: 'external input also captured' });
    }
});
...
export const researchRunDescriptionSchema = z.discriminatedUnion('version', [
    ...
    if (value.version === 2 && value.checkpoint.snapshot)
        ctx.addIssue({ code: 'custom', message: 'scoped checkpoints require plan-bound run v3' });
});
const resultFields = {
