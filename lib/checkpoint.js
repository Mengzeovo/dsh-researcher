...
import { preparedPlanRunResultSchema } from "./schema.js";
...
export class GitCheckpointProvider {
    ...
    async finish(session, input, requestKey, prepared, signal, validate, beforeCapture) {
        try {
            ...
            const outputCommit = await this.commit(repo, outputTree, frozenInput.inputCommit, message({ version: original.version === 3 ? 2 : 1, type: 'dsh-research-output', requestKey, prepared: original, checkpoint: body }), original.finishedAt);
