...
import { appendStateText, parseRunLog, parseStateLog, renderClosedRun, renderOpenRun } from "./jsonl.js";
import { RECORD_MAX_BYTES, SESSION_INDEX_MAX_BYTES, encodeSessionId, nowIso, normalizeProjectRelativePath, parseGoalMarkdown, parseJsonText, parseResearchId, parseRunId, renderGoalMarkdown, researchGlossarySchema, researchRunDescriptionSchema, researchRunResultSchema, researchSessionIndexSchema, researchStateSchema, stableJsonLine, truncateLabel, } from "./schema.js";
class FifoMutex {
...
export class ResearchStore {
    ...
    async createTarget(session, request, signal) {
        ...
        return await this.mutex(session, id).run(async () => {
            ...
            try {
                ...
                await this.createText(session, `${stagingRoot}/session/${encodeSessionId(String(session.id))}.json`, `${JSON.stringify(index, null, 2)}\n`, policy, signal);
    ...
    async resumeState(session, idInput, signal) {
        ...
        return await this.mutex(session, id).run(async () => {
            const target = await this.readTarget(session, id, signal);
            if (target.state.status !== 'paused' && target.state.status !== 'blocked')
                return target;
