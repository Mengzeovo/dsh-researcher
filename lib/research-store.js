...
export class ResearchStore {
    ...
    async startRun(session, idInput, request, signal) {
        ...
        return await (await this.mutex(session, id)).run(async () => {
            ...
            const checkpoint = await this.checkpoints.start(session, id, runId, base.createdAt, reproduction, signal, captured => {
                // Validate the COMPLETE description before the provider publishes an input ref.
                renderOpenRun(researchRunDescriptionSchema.parse({
                    ...base, version: 3, baseStateRevision: target.state.revision, checkpoint: captured, planRef: selected,
                }));
            });
            try {
