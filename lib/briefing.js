...
export class ResearchBriefings {
    ...
    constructor(ctx) {
        ...
        ctx.effect(function* () {
            ...
            ctx.on('agent/inbox/claimed', ({ agent, message, turn }) => {
                ...
                if (reservation?.message.id === message.id)
                    reservation.turn = turn;
            });
            ctx.on('agent/inbox/inserted', ({ agent, message }) => {
                const reservation = owner.reservations.get(agent);
                if (reservation === undefined || reservation.initialIds.has(message.id) || isParked(message))
                    return;
                if (message.source.kind === 'goal' || isBriefing(message))
                    owner.remove(agent, message.id);
                else
            ...
            ctx.on('agent/pre-step', async (proposal, next) => {
                ...
                if (reservation === undefined)
                    return await owner.admitParked(agent, messages, next);
                const deferred = (message) => message.source.kind === 'user'
            ...
            ctx.on('session/event', (session, event) => {
                for (const [agent, reservation] of owner.reservations) {
                    if (agent.session !== session || reservation.turn === undefined)
                        continue;
                    ...
                    if (event.type === 'turn/end' && event.data.turn === reservation.turn) {
                        ...
                        queueMicrotask(() => {
                            if (owner.reservations.get(agent) === reservation)
                                owner.retire(agent);
                        });
                    }
                }
    ...
    busy(agent) { return this.reservations.has(agent); }
    /** Reserve before waking the loop; callers must not await between context injection and this call. */
    queue(agent) {
        if (this.stopping)
            throw new Error('Research briefing service is disposing.');
    ...
    park(agent, messages) {
        ...
        if (reservation !== undefined && marker !== undefined)
            reservation.parkedMarker = marker;
        const pending = [...marker === undefined ? [] : [marker], ...inputs.filter(message => !isParked(message))];
        for (const message of pending) {
            if (message.source.kind !== 'user')
                reservation?.parkedIds.add(message.id);
            ...
            if (pending.some(item => item.id === message.id))
                continue;
            this.remove(agent, message.id);
            agent.inbox.append(target, message);
        }
    }
    async admitParked(agent, messages, next) {
        const marked = messages.some(isParked);
        if (!marked && !messages.some(isBriefing))
            return await next();
        ...
        if (!human) {
            ...
            if (!authorized) {
                this.park(agent, [...messages, ...decision.messages]);
                return { kind: 'reject' };
            }
        }
