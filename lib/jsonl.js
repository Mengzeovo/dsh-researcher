...
export function parseRunLog(runId, text) {
    ...
    if (isCheckpointRunDescription(description.data)) {
        ...
        if ([cp.inputCommit, cp.inputTree, cp.baseHead].some(oid => oid.length !== width))
            invalidRecord('checkpoint object format mismatch');
    }
    ...
    if (desc.version !== closed.version)
        invalidRecord(`runs/${runId}.jsonl record versions disagree`);
    if (isCheckpointRunDescription(desc) && isCheckpointRunResult(closed)) {
        ...
        if ([after.inputCommit, after.outputCommit, after.inputTree, after.outputTree].some(oid => oid.length !== width))
            invalidRecord('checkpoint object format mismatch');
        if ((before.snapshot !== undefined) !== (after.snapshot !== undefined)
            || (after.snapshot && (after.snapshot.baseHead !== before.baseHead || after.snapshot.baseHead.length !== width
                || after.snapshot.deleted.some(file => !before.files.includes(file)))))
            invalidRecord('scoped checkpoint overlay identity disagrees with its description');
