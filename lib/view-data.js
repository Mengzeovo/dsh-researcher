...
import { parsePlanDirectoryName, planDirectory, planLedgerPath, planVersionFile, planVersionPath, verifyPlanLedgerDocument } from "./plan-records.js";
import { runPath } from "./record-store.js";
...
export async function readResearchViewData(records, context, id, config, signal) {
    ...
    const observe = (file) => {
        ...
        if (bytes > config.maxDataBytes)
            throw new ResearcherError('research view input exceeds its configured byte limit', 'RESEARCH_OVERSIZED');
        version.update(JSON.stringify([file.relativePath, file.text]));
    };
    const reserve = (amount = 1) => {
        count += amount;
        if (count > config.maxRecords)
            throw new ResearcherError('research view input exceeds its configured record limit', 'RESEARCH_OVERSIZED');
    };
    const diagnostics = [];
    const diagnose = (error, file, planId) => {
        signal?.throwIfAborted();
        if (bytes > config.maxDataBytes || count > config.maxRecords)
            throw error;
        if (!(error instanceof ResearcherError) && !(error instanceof FsError))
            throw error;
        const diagnostic = { code: error.code, message: error.message, path: file, ...(planId === undefined ? {} : { planId }) };
        diagnostics.push(diagnostic);
        version.update(JSON.stringify(diagnostic));
    };
    const planEntries = await records.listPlanEntries(context, id, signal);
    const goal = await records.readGoal(context, id, signal);
    observe(goal);
    const stateLog = await records.readStateLog(context, id, signal, config.maxDataBytes - bytes);
    observe(stateLog);
    const state = stateLog.value.states.at(-1);
    const plans = [];
    const runs = [];
    const planDirectories = planEntries.filter(entry => entry.type === 'directory').map(entry => parsePlanDirectoryName(entry.name)).filter((value) => value !== undefined).sort((a, b) => a - b);
    ...
    for (const entry of [...planEntries].sort((a, b) => a.name.localeCompare(b.name))) {
        ...
        let ledger;
        try {
            ledger = await records.readPlanLedger(context, id, planId, signal, config.maxDataBytes - bytes);
        }
        catch (error) {
            diagnose(error, planLedgerPath(id, planId), planId);
            continue;
        }
        observe(ledger);
        reserve(ledger.value.entries.length);
        ...
        for (const row of ledger.value.entries) {
            try {
                ...
                diagnose(error, planVersionPath(id, planId, row.revision), planId);
            }
        }
        try {
            ...
            for (const file of files) {
                if (!/^v[0-9]+\.md$/u.test(file.name) || names.has(file.name))
                    continue;
            ...
            diagnose(error, planDirectory(id, planId), planId);
        }
    }
    ...
    for (const entry of [...runEntries].sort((a, b) => a.name.localeCompare(b.name))) {
        if (!entry.name.endsWith('.jsonl') || !UUID_PATTERN.test(entry.name.slice(0, -6)))
            continue;
        ...
        try {
            ...
            diagnose(error, runPath(id, runId));
        }
    }
    signal?.throwIfAborted();
