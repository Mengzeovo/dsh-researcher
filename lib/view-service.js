...
import { researchViewConfigSchema } from "./view-config.js";
import { projectResearchView } from "./view-projection.js";
import { researchWorkflow } from "./view-workflow.js";
...
export class ResearchViewService extends TypertRemoteService {
    ...
    constructor(ctx, config) {
        ...
        this.config = config;
        if (!config.enabled)
            throw new ResearcherError('research view is not enabled', 'RESEARCH_VIEW_UNAVAILABLE');
        ctx.on('researcher/changed', ({ workspaceRoot, researchId }) => {
            for (const [key, entry] of this.cache) {
                if (entry.scope.workspaceRoot === workspaceRoot && entry.scope.researchId === researchId)
                    this.evict(key);
            }
        });
        ctx.effect(() => async () => {
    ...
    getView(request, signal) {
        return this.execute(signal, async (active) => {
            ...
            if (entry === undefined)
                entry = { scope, recordVersion: data.recordVersion, projection, artifacts: new Map(), lifetime: new AbortController() };
            else
    ...
    getViewNode(request, signal) {
        return this.execute(signal, async (active) => {
            const entry = await this.current(request, active);
            const detail = entry.projection.details.get(request.nodeId);
            if (detail === undefined)
                throw new ResearcherError('node is not present on this research view page', 'RESEARCH_NOT_FOUND');
    ...
    renderView(request, signal) {
        return this.execute(signal, async (active) => {
            const entry = await this.current(request, active);
            if (entry.projection.snapshot.nodes.length === 0)
                throw new ResearcherError('an empty research view has no diagram', 'RESEARCH_VIEW_PAGE');
            ...
            let artifact;
            try {
                artifact = await this.ctx.archify.renderWorkflow({ spec: researchWorkflow(entry.projection.snapshot, request.locale), theme: request.theme, locale: request.locale }, renderSignal);
            }
            catch (error) {
            ...
            renderSignal.throwIfAborted();
            if (await this.current(request, renderSignal) !== entry)
                throw this.stale();
            const revision = createHash('sha256').update(JSON.stringify([request.snapshotId, artifact.specSha256, artifact.engineFingerprint, request.theme, request.locale])).digest('hex');
    ...
    async *watchView(request, signal) {
        ...
        let pending = { targetToken: scope.targetToken };
        let wake;
        const dispose = this.ctx.on('researcher/changed', ({ workspaceRoot, researchId }) => {
            if (workspaceRoot !== scope.workspaceRoot || (scope.researchId !== null && researchId !== scope.researchId))
                return;
    ...
    async scope(sessionId, signal) {
        ...
        try {
            ...
            if (cwd === undefined)
                throw new ResearcherError('observed Session has no workspace', 'RESEARCH_PATH_INVALID');
            const projection = observation.projections?.values.researcherBinding;
            if (projection === undefined)
                throw new ResearcherError('research binding projection is unavailable on the observed Session', 'RESEARCH_VIEW_UNAVAILABLE');
            if (projection.failure !== null)
                throw new ResearcherError(projection.failure, 'RESEARCH_INVALID_RECORD');
    ...
    async current(request, signal) {
        ...
        if (entry === undefined || scope.researchId === null || entry.scope.targetToken !== scope.targetToken)
            throw this.stale();
    ...
    targetToken(workspaceRoot, id) {
        return createHmac('sha256', this.salt).update(JSON.stringify([workspaceRoot, id])).digest('hex');
    }
    stale() { return new ResearcherError('research view snapshot is stale; refresh this page', 'RESEARCH_VIEW_STALE'); }
    evict(key) {
        this.cache.get(key)?.lifetime.abort(this.stale());
        this.cache.delete(key);
    }
    assertSize(value, limit, name) {
        if (Buffer.byteLength(JSON.stringify(value), 'utf8') > limit)
            throw new ResearcherError(name + ' exceeds its configured response byte limit', 'RESEARCH_OVERSIZED');
    }
