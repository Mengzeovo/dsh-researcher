...
export function projectResearchView(data, targetToken, request, config) {
    ...
    if (versionPage >= versionPages)
        throw new ResearcherError('requested version page does not exist', 'RESEARCH_VIEW_PAGE');
    ...
    const snapshotId = createHash('sha256').update(JSON.stringify({ format: 1, targetToken, records: data.recordVersion, selection, versionsPerPage: config.versionsPerPage, runsPerVersionPage: config.runsPerVersionPage })).digest('hex');
    const snapshot = {
