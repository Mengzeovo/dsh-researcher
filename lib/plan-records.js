...
import { RECORD_MAX_BYTES, assertUtf8Bound, parseResearchId, stableJsonLine } from "./schema.js";
import { planContentInputSchema, planDocumentSchema, planLedgerEntrySchema, planMetadataSchema, planNumberSchema, } from "./plan-schema.js";
function parseValue(subject, schema, value) {
    const parsed = schema.safeParse(value);
    if (!parsed.success)
        invalidRecord(`${subject} does not match schema: ${z.prettifyError(parsed.error)}`);
    return parsed.data;
}
export function formatPlanNumber(value) {
...
export function parsePlanDirectoryName(name) {
    if (!/^[0-9]+$/u.test(name))
        return undefined;
    const number = Number(name);
    if (!planNumberSchema.safeParse(number).success)
        return undefined;
    return formatPlanNumber(number) === name ? number : undefined;
}
export function planRoot(id) {
    return `.research/goal/${parseResearchId(id)}/plan`;
}
export function planDirectory(id, planId) {
    return `${planRoot(id)}/${formatPlanNumber(planId)}`;
}
export function planVersionFile(revision) {
    return `v${formatPlanNumber(revision)}.md`;
}
export function planVersionPath(id, planId, revision) {
    return `${planDirectory(id, planId)}/${planVersionFile(revision)}`;
}
export function planLedgerPath(id, planId) {
    return `${planDirectory(id, planId)}/versions.jsonl`;
}
export function hashPlanMarkdown(markdown) {
    if (!markdown.isWellFormed())
        invalidRecord('plan Markdown must be well-formed UTF-8 text');
    return createHash('sha256').update(markdown, 'utf8').digest('hex');
}
/** Parsing never normalizes the bytes hashed by the independent ledger. */
export function parsePlanDocument(markdown, expected) {
    assertUtf8Bound('plan Markdown', markdown, RECORD_MAX_BYTES);
    const sha256 = hashPlanMarkdown(markdown);
    const match = /^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/u.exec(markdown);
    if (match === null)
        invalidRecord('plan Markdown must begin with a complete YAML front matter block');
...
export function renderPlanDocument(planId, revision, input, createdAt, evidence = [], schemaVersion = 2) {
...
export function planContentMatches(document, input) {
    const evidence = document.metadata.schema_version === 2 ? document.metadata.based_on_runs : [];
    if (!evidenceMatchesInput(evidence, input))
        return false;
...
export function planLedgerEntry(document) {
...
export function parsePlanLedger(planId, text) {
    formatPlanNumber(planId);
    const lines = text.split('\n');
    if (text.endsWith('\n'))
        lines.pop();
    if (lines.length === 0 || text.length === 0)
        invalidRecord('versions.jsonl must contain at least one committed plan revision');
    const entries = lines.map((line, index) => {
        const subject = `versions.jsonl line ${index + 1}`;
        if (line.length === 0)
            invalidRecord(`${subject} is an empty record`);
        ...
        if (entry.plan_id !== planId)
            invalidRecord(`${subject} belongs to a different plan`);
...
export function renderPlanLedger(entries) {
...
export function appendPlanLedgerText(parsed, entry) {
    const first = parsed.entries[0];
    if (first === undefined)
        invalidRecord('cannot append to an empty committed plan ledger');
...
export function verifyPlanLedgerDocument(document, entry) {
...
//# sourceMappingURL=plan-records.js.map